...
const mockDb = actualDb;
const mockStripeService = actualStripeService;
const mockLogger = actualLogger;
describe('SubscriptionService', () => {
    let subscriptionService;
    // Mocks for Drizzle chainable methods (Manual Setup)
    let mockSelect;
    let mockFrom;
    let mockWhere; // This will return the Promise
    let mockLimit; // <--- Re-declare mockLimit
    let mockInsert;
    let mockValues;
    let mockOnConflictDoUpdate; // This will return the Promise
    let mockUpdate; // Add mock for update
    let mockSet; // Add mock for set
    beforeEach(() => {
        vi.resetAllMocks();
        ...
        mockUpdate = mockDb.update; // Initialize mockUpdate
    ...
    describe('getPlans', () => {
        ...
        it('should fetch plans from Stripe and sync to DB if DB is empty', async () => {
            ...
            // 2. Mock Stripe call
            mockStripeService.listPlans.mockResolvedValueOnce([mockStripePlan1, mockStripePlan2]);
        ...
        it('should handle errors when fetching from DB', async () => {
            ...
            await expect(subscriptionService.getPlans()).rejects.toThrow(new AppError('Failed to fetch subscription plans', 500));
        ...
        it('should handle errors when fetching from Stripe during sync', async () => {
            ...
            // Check type and message in one assertion
            await expect(subscriptionService.getPlans()).rejects.toThrow(new AppError('Failed to fetch subscription plans', 500) // Outer catch block throws this
            );
        ...
        it('should filter out inactive plans from Stripe', async () => {
            ...
            mockWhere.mockResolvedValueOnce([]);
            // 2. Mock Stripe call
            mockStripeService.listPlans.mockResolvedValueOnce([mockActiveStripePlan, mockInactiveStripePlan]);
        ...
        it('should filter out Stripe plans with deleted products', async () => {
            const mockStripePlanWithDeletedProduct = {
                ...
                recurring: { interval: 'month' },
                product: { id: 'prod_deleted', deleted: true, object: 'product' }
            };
            const mockActiveStripePlan = {
                id: 'price_stripe_1', active: true, type: 'recurring', unit_amount: 1000,
                recurring: { interval: 'month' },
                product: { id: 'prod_1', name: 'Active Plan', metadata: { tier: 'basic' }, deleted: false, object: 'product' }
            };
            const mockSyncedPlan = [
                { id: 'price_stripe_1', name: 'Active Plan', description: undefined, price: '10', interval: 'month', tier: 'basic', studentLimit: 3, moduleLimit: 3, customModuleLimit: 1, isActive: true, createdAt: expect.any(Date), updatedAt: expect.any(Date) }
            ];
            ...
            mockWhere.mockResolvedValueOnce([]);
            // 2. Mock Stripe call
            mockStripeService.listPlans.mockResolvedValueOnce([mockStripePlanWithDeletedProduct, mockActiveStripePlan]);
            // 3. Mock insert (only expects one call)
            mockOnConflictDoUpdate.mockResolvedValue({ rowCount: 1 });
            // 4. Second DB select returns the single synced plan (mock where)
            mockWhere.mockResolvedValueOnce(mockSyncedPlan);
    ...
    describe('getCurrentSubscription', () => {
        ...
        it('should throw an error if no subscription and no free plan is found', async () => {
            ...
            await expect(subscriptionService.getCurrentSubscription(userId)).rejects.toThrow(new AppError('No free plan found', 404));
            expect(mockSelect).toHaveBeenCalledTimes(2);
            expect(mockLimit).toHaveBeenCalledTimes(2);
        });
        it('should throw an error if subscription exists but associated plan is not found', async () => {
            ...
            mockLimit.mockResolvedValueOnce([]);
            // Check type and message in one assertion - Expect the specific internal error
            await expect(subscriptionService.getCurrentSubscription(userId)).rejects.toThrow(new AppError('Subscription plan not found', 404));
            expect(mockSelect).toHaveBeenCalledTimes(2);
            expect(mockLimit).toHaveBeenCalledTimes(2);
        });
    ...
    describe('createCheckoutSession', () => {
        ...
        it('should create a checkout session for a new user (no existing subscription or stripe ID)', async () => {
            ...
            mockStripeService.createCustomer.mockResolvedValueOnce(mockStripeCustomer);
            ...
            mockStripeService.createCheckoutSession.mockResolvedValueOnce(mockStripeSession);
            ...
            expect(mockStripeService.createCustomer).toHaveBeenCalledWith(mockProfile.email, mockProfile.fullName, { userId });
            ...
            expect(mockStripeService.createCheckoutSession).toHaveBeenCalledWith(mockStripeCustomer.id, planId, `${mockClientUrl}/subscription/success?session_id={CHECKOUT_SESSION_ID}`, `${mockClientUrl}/subscription/cancel`);
        });
        ...
        it('should use existing stripeCustomerId from profile if no subscription exists', async () => {
            ...
            expect(mockStripeService.createCheckoutSession).toHaveBeenCalledWith('cus_existing_profile', // Uses ID from profile
            planId, expect.any(String), expect.any(String));
            // REMOVED: No check for db.insert needed
            // expect(mockInsert).not.toHaveBeenCalled(); 
        });
        it('should use existing stripeCustomerId from subscription record', async () => {
            ...
            expect(mockStripeService.createCheckoutSession).toHaveBeenCalledWith('cus_existing_sub', // Uses ID from existing subscription
            planId, expect.any(String), expect.any(String));
            // REMOVED: No check for db.insert needed
            // expect(mockInsert).not.toHaveBeenCalled(); 
        });
        it('should throw an error if user is already actively subscribed to the same plan', async () => {
            ...
            // Assertions
            await expect(subscriptionService.createCheckoutSession(userId, planId)).rejects.toThrow(new AppError('User is already actively subscribed to this plan', 400));
            expect(mockStripeService.createCustomer).not.toHaveBeenCalled();
        ...
        it('should ALLOW checkout if user has an INCOMPLETE subscription for the same plan', async () => {
            ...
            expect(session).toEqual(mockStripeSession);
            expect(mockStripeService.createCustomer).not.toHaveBeenCalled();
            expect(mockStripeService.createCheckoutSession).toHaveBeenCalledWith('cus_incomplete', // Uses ID from incomplete subscription
            planId, expect.any(String), expect.any(String));
            // REMOVED: No check for db.insert needed
            // expect(mockInsert).not.toHaveBeenCalled(); 
        });
        it('should throw an error if the plan is not found', async () => {
            ...
            await expect(subscriptionService.createCheckoutSession(userId, planId)).rejects.toThrow(new AppError('Subscription plan not found', 404));
        });
        it('should throw an error if the user profile is not found', async () => {
            ...
            await expect(subscriptionService.createCheckoutSession(userId, planId)).rejects.toThrow(new AppError('User not found', 404));
        });
        ...
        it('should handle errors during Stripe checkout session creation', async () => {
            ...
            await expect(subscriptionService.createCheckoutSession(userId, planId)).rejects.toThrow(new AppError('Failed to create checkout session', 500));
    ...
    describe('cancelSubscription', () => {
        ...
        it('should successfully cancel an active subscription', async () => {
            ...
            mockStripeService.cancelSubscription.mockResolvedValue(mockStripeCanceledSubscription);
        ...
        it('should throw an error if the user has no subscription', async () => {
            ...
            await expect(subscriptionService.cancelSubscription(userId)).rejects.toThrow(new AppError('No active subscription found', 404) // Correct error message and code
            );
        ...
        it('should throw an error if the found subscription is not active', async () => {
            ...
            await expect(subscriptionService.cancelSubscription(userId)).rejects.toThrow(new AppError('Subscription is not active', 400) // Correct error message and code
            );
            // Verify DB select call was made
            expect(mockSelect).toHaveBeenCalled();
            expect(mockFrom).toHaveBeenCalledWith(schema.customerSubscriptions);
        ...
        it('should throw an error and log if Stripe cancellation fails', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith(`Error canceling subscription for user ${userId}:`, // Correct log message format
            stripeError);
    ...
    describe('reactivateSubscription', () => {
        ...
        it('should successfully reactivate a subscription set to cancel', async () => {
            ...
            mockStripeService.reactivateSubscription.mockResolvedValue(mockStripeReactivatedSubscription);
            // 3. Call the service method
        ...
        it('should throw an error if no subscription is found for the user', async () => {
            ...
            mockLimit.mockResolvedValue([]);
            await expect(subscriptionService.reactivateSubscription(userId)).rejects.toThrow(new AppError('No subscription found for user', 404));
            // Verify DB select was called
            expect(mockLimit).toHaveBeenCalledWith(1);
        ...
        it('should throw an error if the subscription status is not active', async () => {
            const mockInactiveSubscription = { ...mockReactivatableSubscription, status: 'canceled' }; // e.g., canceled
            // Mock DB select returns the inactive subscription
            mockLimit.mockResolvedValue([mockInactiveSubscription]);
            await expect(subscriptionService.reactivateSubscription(userId)).rejects.toThrow(new AppError('Subscription cannot be reactivated', 400));
            // Verify DB select was called
            expect(mockLimit).toHaveBeenCalledWith(1);
            // Verify Stripe was not called
            expect(mockStripeService.reactivateSubscription).not.toHaveBeenCalled();
        ...
        it('should throw an error and log if Stripe reactivation fails', async () => {
            ...
            expect(mockStripeService.reactivateSubscription).toHaveBeenCalledWith(mockReactivatableSubscription.id);
            // Verify logger
            expect(mockLogger.error).toHaveBeenCalledWith(`Error reactivating subscription for user ${userId}:`, stripeError);
            // Verify DB update was NOT called
            expect(mockUpdate).not.toHaveBeenCalled();
        });
    });
    describe('getPaymentHistory', () => {
        ...
        it('should return payment history from Stripe if customer ID is found', async () => {
            ...
            mockStripeService.getPaymentHistory.mockResolvedValue(mockStripePaymentHistoryResponse);
            // 3. Call the service method
        ...
        it('should handle errors during database lookup', async () => {
            ...
            await expect(subscriptionService.getPaymentHistory(userId)).rejects.toThrow(new AppError('Failed to fetch payment history', 500));
