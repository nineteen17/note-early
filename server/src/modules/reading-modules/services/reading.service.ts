...
import { ReadingLevel, ModuleType, UserRole, Genre, Paragraph, Language, VocabularyEntry } from '@shared/types';
import { eq, and, count, asc, SQL, or, desc } from 'drizzle-orm';
import { AppError } from '@/utils/errors';
import { SubscriptionService } from '@/modules/subscription/services/subscription.service';
import { logger } from '@/utils/logger';
import { Vocabulary, ReadingModule } from '@/db/schema';

// Input type for creating vocabulary
export interface NewVocabularyInput {
  paragraphIndex: number;
  word: string;
  description: string;
}

// Define the structure for input data when creating a module
export interface CreateModuleInput {
  title: string;
  structuredContent: Paragraph[]; // Use structuredContent
  level: ReadingLevel;
  type: ModuleType;
  adminId?: string | null; // Optional: Required only for CUSTOM type
  genre: Genre; // --- ADDED Genre (Mandatory for creation)
  language: Language; // --- ADDED Language (Mandatory for creation)
...
// Define the structure for updating a module (subset of creation)
// Note: Updating structuredContent implies recalculating paragraphCount
export type UpdateModuleInput = Partial<Omit<CreateModuleInput, 'type' | 'adminId'>>;
...
export class ReadingModuleService {
  ...
  // --- CREATE --- //
  async createModule(input: CreateModuleInput): Promise<schema.ReadingModule> {
    ...
    if (input.type === ModuleType.CUSTOM) {
      ...
      if (currentCustomModuleCount >= plan.customModuleLimit) {
        throw new AppError(
          `Custom module limit (${plan.customModuleLimit}) for your current plan ('${plan.tier}') has been reached. Please upgrade to create more custom modules.`,
          403 // Forbidden
        );
      }
  ...
  // --- RETRIEVE --- //
  async getModuleById(id: string): Promise<schema.ReadingModule | null> {
    try {
  ...
  async getActiveModules(requestingUserId?: string): Promise<schema.ReadingModule[]> {
    try {
            ...
            where: and(
              ...
              or(
                ...
                and(
                  ...
                  eq(schema.readingModules.adminId, requestingUserId)
                )
              )
            )
          });
  ...
  async getModulesByAdmin(adminId: string): Promise<schema.ReadingModule[]> {
     if (!adminId) {
  ...
  // --- UPDATE --- //
  async updateModule(id: string, updates: UpdateModuleInput, requestingUserId: string): Promise<schema.ReadingModule> {
    ...
    let updatesWithCount: Partial<schema.ReadingModule & { authorFirstName?: string | null; authorLastName?: string | null }> = { ...updates };
    ...
    if (!isSuperAdmin) {
        delete (updatesWithCount as any).type;
        delete (updatesWithCount as any).adminId;
    }
  ...
  async superAdminUpdateModule(id: string, updates: UpdateModuleInput): Promise<schema.ReadingModule> {
  ...
  // --- DELETE (Soft) --- //
  async deleteModule(id: string, requestingUserId: string): Promise<schema.ReadingModule> {
    // 1. Fetch the module first to check existence and ownership
    const existingModule = await this.getModuleById(id);
   ...
   */
  async getAllModulesForAdmin(): Promise<schema.ReadingModule[]> {
    try {
      const modules = await this.db.query.readingModules.findMany({
   ...
   * Orders by most recently updated.
   */
  async getCuratedModulesForAdmin(): Promise<schema.ReadingModule[]> {
    try {
      const modules = await this.db.query.readingModules.findMany({
   ...
   * Orders by most recently updated.
   */
  async getCustomModulesForAdmin(): Promise<(schema.ReadingModule & { adminFullName: string | null })[]> {
    try {
      ...
      return modules as (schema.ReadingModule & { adminFullName: string | null })[];
    } catch (error) {
  ...
  async getModuleParagraph(moduleId: string, paragraphIndex: number): Promise<Paragraph | null> {
    ...
    try {
      ...
      const paragraph = (moduleContent.structuredContent as Paragraph[]).find(
   ...
   */
  async createVocabularyEntry(moduleId: string, userId: string, data: NewVocabularyInput): Promise<schema.Vocabulary> {
     try {
   ...
   * Requires admin permission (checked by verifying module ownership).
   */
  async updateVocabularyEntry(vocabularyId: string, userId: string, updates: Partial<NewVocabularyInput>): Promise<schema.Vocabulary> {
    try {
        // 1. Fetch the existing entry and its module
        const existingEntry: (Vocabulary & { module: ReadingModule | null }) | undefined = 
          await this.db.query.vocabulary.findFirst({
   ...
   * Requires admin permission (checked by verifying module ownership).
   */
  async deleteVocabularyEntry(vocabularyId: string, userId: string): Promise<void> {
     try {
   ...
   */
  async getVocabularyForModule(moduleId: string): Promise<schema.Vocabulary[]> {
    try {
   ...
   */
  async getVocabularyForParagraph(moduleId: string, paragraphIndex: number): Promise<schema.Vocabulary[]> {
    try {
         if (paragraphIndex <= 0) {
         ...
         const entries = await this.db.query.vocabulary.findMany({
            where: and(
                eq(schema.vocabulary.moduleId, moduleId),
                eq(schema.vocabulary.paragraphIndex, paragraphIndex)
            ),
