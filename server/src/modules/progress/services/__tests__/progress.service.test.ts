...
import { ProgressService, CreateProgressInput, UpdateProgressInput } from '../progress.service'; // Import the service
import { AppError } from '@/utils/errors'; // Import AppError for error handling
import { eq, and, asc, desc } from 'drizzle-orm'; // Import eq and asc for mocking where clauses
...
describe('ProgressService', () => {
  ...
  let mockDb: Mocked<typeof dbInstance>;
  ...
  const mockModule: Partial<schema.ReadingModule> = { 
    id: moduleId, 
    title: 'Test Module',
    paragraphCount: 10 // Add paragraphCount
  }; 
  const mockProgressRecord: schema.StudentProgress = {
    id: progressId,
    studentId,
    moduleId,
    completed: false,
  ...
  beforeEach(() => {
    vi.resetAllMocks();
    mockDb = dbInstance as Mocked<typeof dbInstance>; 
    ...
    if (mockDb.query?.studentProgress?.findFirst) {
      (mockDb.query.studentProgress.findFirst as Mock).mockReset();
    }
    if (mockDb.query?.profiles?.findFirst) {
      (mockDb.query.profiles.findFirst as Mock).mockReset();
    }
    if (mockDb.query?.readingModules?.findFirst) {
      (mockDb.query.readingModules.findFirst as Mock).mockReset();
    }
    if (mockDb.query?.studentProgress?.findMany) {
      (mockDb.query.studentProgress.findMany as Mock).mockReset();
    }
    if (mockDb.query?.paragraphSubmissions?.findMany) {
      (mockDb.query.paragraphSubmissions.findMany as Mock).mockReset();
    }
  ...
  describe('startProgress', () => {
    ...
    it('should create a new progress record if none exists', async () => {
      ...
      (mockDb.query.studentProgress.findFirst as Mock).mockResolvedValueOnce(undefined);
      // 2. Mock student check to return a student
      (mockDb.query.profiles.findFirst as Mock).mockResolvedValueOnce(mockStudent);
      // 3. Mock module check to return a module
      (mockDb.query.readingModules.findFirst as Mock).mockResolvedValueOnce(mockModule as schema.ReadingModule);
    ...
    it('should return existing progress record if one exists', async () => {
      ...
      (mockDb.query.studentProgress.findFirst as Mock).mockResolvedValueOnce(mockProgressRecord);
    ...
    it('should throw an error if student not found', async () => {
      ...
      (mockDb.query.profiles.findFirst as Mock).mockResolvedValueOnce(undefined);

      await expect(progressService.startProgress(input)).rejects.toThrow(
    ...
    it('should throw an error if module not found', async () => {
      ...
      (mockDb.query.readingModules.findFirst as Mock).mockResolvedValueOnce(undefined);

      await expect(progressService.startProgress(input)).rejects.toThrow(
        new AppError('Reading module not found', 404)
      );
  ...
  describe('getStudentModuleProgress', () => {
    ...
    it('should return null if no progress record found', async () => {
      ...
      (mockDb.query.studentProgress.findFirst as Mock).mockResolvedValueOnce(null);
  ...
  describe('getAllStudentProgress', () => {
    it('should return all progress records for a student', async () => {
      ...
      (mockDb.query.studentProgress.findMany as Mock).mockResolvedValueOnce(progressRecords);
    ...
    it('should throw an error if student not found', async () => {
      ...
      await expect(progressService.getAllStudentProgress(studentId)).rejects.toThrow(
        new AppError('Student not found', 404)
      );
  ...
  describe('getAllModuleProgress', () => {
    ...
    it('should throw an error if module not found', async () => {
      ...
      (mockDb.query.readingModules.findFirst as Mock).mockResolvedValueOnce(null);

      await expect(progressService.getAllModuleProgress(moduleId)).rejects.toThrow(
        new AppError('Reading module not found', 404)
      );
  ...
  describe('submitParagraphSummary', () => {
    ...
    const mockSubmission: schema.ParagraphSubmission = {
    ...
    it('should successfully submit a paragraph summary', async () => {
      ...
      (mockDb.query.readingModules.findFirst as Mock).mockResolvedValueOnce({ paragraphCount: 5, id: moduleId } as schema.ReadingModule);
    ...
    it('should mark module as completed when submitting the last paragraph', async () => {
      ...
      (mockDb.query.readingModules.findFirst as Mock).mockResolvedValueOnce({ paragraphCount: 1, id: moduleId } as schema.ReadingModule);
    ...
    it('should throw an error if module is already completed', async () => {
      ...
      (mockDb.query.studentProgress.findFirst as Mock).mockResolvedValueOnce(completedProgress);
    ...
    it('should throw an error if module not found or has invalid paragraph count', async () => {
      ...
      (mockDb.query.studentProgress.findFirst as Mock).mockResolvedValueOnce(mockProgressRecord);
      (mockDb.query.readingModules.findFirst as Mock).mockResolvedValueOnce({ paragraphCount: 0, id: moduleId } as schema.ReadingModule);
    ...
    it('should handle database errors gracefully during insert', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        'Error submitting paragraph summary:',
        expect.objectContaining({ input: paragraphSummaryInput, error: expect.any(Error) })
      );
    });
  ...
  describe('getStudentProgressDetails', () => {
    it('should return progress and submissions when found', async () => {
      ...
      const mockProgress = {
        ...
        teacherFeedbackAt: null,
        score: null,
      } as schema.StudentProgress;

      // Sample paragraph submissions
      const mockSubmissions: schema.ParagraphSubmission[] = [
        {
          id: 'submission-1',
          studentProgressId: progressId,
          paragraphIndex: 1,
      ...
      (mockDb.query.studentProgress.findFirst as Mock).mockResolvedValueOnce(mockProgress);
      
      // 2. Mock the submissions query
      (mockDb.query.paragraphSubmissions.findMany as Mock).mockResolvedValueOnce(mockSubmissions);
    ...
    it('should handle database errors gracefully', async () => {
      ...
      (mockDb.query.studentProgress.findFirst as Mock).mockRejectedValueOnce(new Error('Database error'));
      ...
      // Verify error was logged
      expect(logger.error).toHaveBeenCalledWith(
        'Error getting student progress details:',
        expect.objectContaining({ studentId, moduleId, error: expect.any(Error) })
      );
    });
  });
}); 