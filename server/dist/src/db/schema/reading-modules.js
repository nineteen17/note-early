...
export const readingModules = pgTable('reading_modules', {
    ...
    structuredContent: jsonb('structured_content').$type().notNull(),
    paragraphCount: integer('paragraph_count').notNull(),
    level: integer('level').$type().notNull(),
    type: varchar('type', { length: 50 }).$type().notNull(),
    genre: varchar('genre', { length: 50 }).$type().notNull(),
    language: varchar('language', { length: 10 }).$type().notNull(),
