...
describe('AuthService', () => {
    ...
    beforeEach(() => {
        ...
        bcrypt.hash = vi.fn().mockImplementation(async () => Promise.resolve('hashed_value'));
        bcrypt.compare = vi.fn().mockImplementation(async () => Promise.resolve(true));
    ...
    describe('loginAdmin', () => {
        ...
        it('should login via Supabase, fetch profile, and return profile, user, session', async () => {
            mockSupabase.auth.signInWithPassword.mockResolvedValue({ data: { user: mockUser, session: mockSession }, error: null });
        ...
        it('should throw supabase error if Supabase login fails', async () => {
            ...
            mockSupabase.auth.signInWithPassword.mockResolvedValue({ data: { user: null, session: null }, error: authError });
    ...
    describe('createStudent', () => {
        ...
        const mockPlan = {
            ...
            price: '0',
            interval: 'month',
            tier: 'free', // Use 'as const' for the literal type
            studentLimit: 5,
            moduleLimit: 10,
            customModuleLimit: 1,
            isActive: true,
        ...
        it('should hash the pin, insert student profile with age and readingLevel, and return the new profile', async () => {
            ...
            const paidPlan = {
                ...mockPlan,
                tier: 'home' // Ensure literal type is used
            };
    ...
    describe('loginStudent', () => {
        ...
        it('should return Profile if student found, is STUDENT, and PIN matches', async () => {
            ...
            expect(mockDb.select).toHaveBeenCalled();
            expect(bcrypt.compare).toHaveBeenCalledWith(correctPin, mockStudentProfile.pin);
        ...
        it('should throw error if PIN does not match', async () => {
            ...
            bcrypt.compare = vi.fn().mockImplementation(async () => Promise.resolve(false));
            await expect(authService.loginStudent(studentId, 'wrongPin'))
                .rejects.toThrow('Invalid PIN');
            expect(bcrypt.compare).toHaveBeenCalledWith('wrongPin', mockStudentProfile.pin);
        });
    ...
    describe('resetAdminPassword', () => {
        ...
        const mockUser = { id: adminId, email: 'admin@test.com' }; // Mock Supabase user 
        ...
        it('should verify current password, update via Supabase, fetch profile, and return updated profile', async () => {
            ...
            mockSupabase.auth.updateUser = vi.fn().mockResolvedValue({ data: { user: mockUser }, error: null });
        ...
        it('should throw error if Supabase update fails', async () => {
            ...
            mockSupabase.auth.updateUser = vi.fn().mockRejectedValue(dbError);
            // Corrected expected error message
            await expect(authService.resetAdminPassword(adminId, currentPassword, newPassword))
