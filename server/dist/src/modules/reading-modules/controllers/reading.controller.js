...
import { z } from 'zod';
import { ReadingLevel, ModuleType } from '@shared/types'; // Import DTO
...
const allowedGenres = ['History', 'Adventure', 'Science', 'Non-Fiction', 'Fantasy', 'Biography', 'Mystery', 'Science-Fiction', 'Folktale', 'Custom'];
...
const UpdateVocabularyBodySchema = VocabularyBodySchema.partial().refine((data) => Object.keys(data).length > 0, { message: 'At least one field (paragraphIndex, word, description) must be provided for update' });
...
function mapToReadingModuleDTO(module) {
    return {
        id: module.id,
...
function mapToVocabularyEntryDTO(vocab) {
    return {
        id: vocab.id,
...
export class ReadingModuleController {
    ...
    async createReadingModule(req, res, next) {
        try {
            ...
            const input = {
                ...validatedInput,
                language: validatedInput.language, // Cast from string literal
                adminId: req.user?.id,
    ...
    async getReadingModuleById(req, res, next) {
        try {
    ...
    async getAllActiveReadingModules(req, res, next) {
        try {
    ...
    async getAdminReadingModules(req, res, next) {
        try {
    ...
    async updateReadingModule(req, res, next) {
        try {
            const { id } = IdParamSchema.parse(req.params);
            ...
            const updates = {
                ...validatedUpdates,
                language: validatedUpdates.language ? validatedUpdates.language : undefined,
            };
    ...
    async deleteReadingModule(req, res, next) {
        try {
            const { id } = IdParamSchema.parse(req.params);
            const requestingUserId = req.user?.id;
            if (!requestingUserId) {
    ...
    async createCuratedModule(req, res, next) {
        try {
            ...
            const input = {
                ...curatedData,
                language: curatedData.language, // Cast from string literal
                adminId: null,
    ...
    async superAdminUpdateReadingModule(req, res, next) {
        try {
            const { id } = IdParamSchema.parse(req.params);
    ...
    async superAdminDeleteReadingModule(req, res, next) {
        try {
    ...
    async getModuleParagraph(req, res, next) {
        try {
    ...
    // --- Vocabulary Methods ---
    async createVocabularyEntry(req, res, next) {
        try {
    ...
    async updateVocabularyEntry(req, res, next) {
        try {
    ...
    async deleteVocabularyEntry(req, res, next) {
        try {
            const { vocabularyId } = VocabularyIdParamSchema.parse(req.params);
            const userId = req.user?.id;
            if (!userId) {
    ...
    async getModuleVocabulary(req, res, next) {
        try {
            const { moduleId } = ModuleIdParamSchema.parse(req.params);
    ...
    async getParagraphVocabulary(req, res, next) {
        try {
