...
import { testConnection } from '@/db'; // Import testConnection if needed elsewhere
import * as schema from '@/db/schema'; // Import schemas for types and mocking
import { ReadingModuleService, CreateModuleInput, UpdateModuleInput, NewVocabularyInput } from '../services/reading.service'; // Added NewVocabularyInput
import { ReadingLevel, ModuleType, UserRole, Genre, Paragraph, Language } from '@shared/types'; // Correctly import UserRole enum and Genre, Paragraph, Language
import { AppError } from '@/utils/errors'; // Import AppError for error handling
import { eq, and, count, or, desc, asc } from 'drizzle-orm'; // Import eq for mocking where clauses
import { SubscriptionService } from '@/modules/subscription/services/subscription.service'; // Import SubscriptionService
import { subscriptionPlans } from '@/db/schema'; // Import subscriptionPlans for typing
import { NodePgDatabase } from 'drizzle-orm/node-postgres'; // Import the specific database type for the driver being used
...
vi.mock('@/db', async (importOriginal) => {
  ...
  const actualDbModule = await importOriginal<typeof import('@/db')>();
...
describe('ReadingModuleService', () => {
  let readingModuleService: ReadingModuleService;
  // Use the specific Drizzle database type for the mock
  let mockDb: Mocked<NodePgDatabase<typeof schema>>;
  ...
  const mockReadingModule: schema.ReadingModule = {
  ...
  beforeEach(async () => { // Make beforeEach async if needed for imports
    ...
    mockDb = importedMockDb as unknown as Mocked<NodePgDatabase<typeof schema>>;
    ...
    readingModuleService = new ReadingModuleService(mockDb as any);
    ...
    vi.spyOn(mockDb.query.profiles, 'findFirst').mockResolvedValue({ id: 'default-admin-id', role: UserRole.ADMIN } as Partial<schema.Profile> as any); // Use Partial and cast to any
    ...
    vi.spyOn(mockDb, 'insert').mockImplementation(() => ({ values: mockInsertValues }) as any);
  ...
  describe('createModule', () => {
    it('should create a new curated module successfully', async () => {
      const inputData: CreateModuleInput = {
      ...
      const expectedModule: schema.ReadingModule = {
        id: curatedModuleId,
      ...
      vi.spyOn(mockDb, 'insert').mockImplementationOnce(() => ({ values: mockValues }) as any);
    ...
    it('should create a new custom module successfully with an adminId', async () => {
      ...
      vi.spyOn(mockDb, 'select').mockImplementationOnce(() => ({ from: mockFromCount }) as any);
    ...
    it('should throw an error if required fields are missing (including genre and language)', async () => {
      const inputDataMissingGenre: Partial<CreateModuleInput> = {
      ...
      const inputDataMissingTitle: Partial<CreateModuleInput> = {
        structuredContent: [{ index: 1, text: 'abc' }],
        level: ReadingLevel.LEVEL_1,
        type: ModuleType.CURATED,
        genre: 'History',
      ...
      const inputDataMissingLanguage: Partial<CreateModuleInput> = {
        title: 'Incomplete Module',
        type: ModuleType.CURATED,
        level: ReadingLevel.LEVEL_1,
        structuredContent: [{index: 1, text: 'abc'}],
      ...
      await expect(readingModuleService.createModule(inputDataMissingGenre as CreateModuleInput))
        .rejects.toThrow(new AppError('Missing required fields for module creation.', 400));
      await expect(readingModuleService.createModule(inputDataMissingTitle as CreateModuleInput))
        .rejects.toThrow(new AppError('Missing required fields for module creation.', 400));
      await expect(readingModuleService.createModule(inputDataMissingLanguage as CreateModuleInput))
        .rejects.toThrow(new AppError('Missing required fields for module creation.', 400));
  ...
  describe('getActiveModules', () => {
    it('should return only active modules', async () => {
      const activeModule: schema.ReadingModule = {
        id: 'active-1',
        title: 'Active Module',
      ...
      expect(mockDb.query.readingModules.findMany).toHaveBeenCalledWith({
        where: and(
          eq(schema.readingModules.isActive, true),
          or(
            eq(schema.readingModules.type, ModuleType.CURATED),
            and(
              eq(schema.readingModules.type, ModuleType.CUSTOM),
              eq(schema.readingModules.adminId, testAdminId)
            )
          )
        )
      });
    ...
    it('should return limited curated modules for non-authenticated user', async () => {
      const mockModules: Partial<schema.ReadingModule>[] = [
      ...
      vi.spyOn(mockDb.query.readingModules, 'findMany').mockResolvedValue(mockModules as schema.ReadingModule[]);
    ...
    it('should return curated modules for authenticated user with paid plan', async () => {
      ...
      const expectedFilteredModules: Partial<schema.ReadingModule>[] = [
        { ...mockReadingModule, id: 'c1', type: ModuleType.CURATED, genre: 'History', language: Language.US },
        { ...mockReadingModule, id: 'cust1', type: ModuleType.CUSTOM, adminId: testAdminId, genre: 'Custom', language: Language.UK },
        { ...mockReadingModule, id: 'c2', type: ModuleType.CURATED, genre: 'Science', language: Language.US },
      ];
      // Cast the mock data
      vi.spyOn(mockDb.query.readingModules, 'findMany').mockResolvedValue(expectedFilteredModules as schema.ReadingModule[]);
    ...
    it('should throw AppError if adminId is not provided', async () => {
      await expect(readingModuleService.getModulesByAdmin(null as any))
    ...
    it('should allow SuperAdmin to update any module', async () => {
      ...
      const updates: UpdateModuleInput = { isActive: false, genre: 'Custom', language: Language.UK };
      ...
      vi.spyOn(mockDb.query.readingModules, 'findFirst').mockResolvedValue(existingModule as schema.ReadingModule);
      // Update profile mock
      vi.spyOn(mockDb.query.profiles, 'findFirst').mockResolvedValue({ id: 'super-admin-uuid', role: UserRole.SUPER_ADMIN } as Partial<schema.Profile> as any);
      ...
      vi.spyOn(mockDb, 'update').mockImplementationOnce(() => ({ set: mockSetUpdate }) as any);
    ...
    it('should throw 404 if module to update is not found', async () => {
      ...
      const updates: UpdateModuleInput = { title: 'New Title' };
    ...
    it('should throw 403 if user is not owner and not SuperAdmin', async () => {
      ...
      vi.spyOn(mockDb.query.readingModules, 'findFirst').mockResolvedValue(existingModule as schema.ReadingModule);
      // Update profile mock
      vi.spyOn(mockDb.query.profiles, 'findFirst').mockResolvedValue({ id: testAdminId, role: UserRole.ADMIN } as Partial<schema.Profile> as any);
      const updates: UpdateModuleInput = { title: 'Attempted Update' };
    ...
    it('should recalculate paragraphCount if structuredContent is updated', async () => {
      ...
      const updates: UpdateModuleInput = { structuredContent: newContent };
      ...
      vi.spyOn(mockDb, 'update').mockImplementationOnce(() => ({ set: mockSetRecalc }) as any);
  ...
  describe('deleteModule', () => {
    ...
    const existingModule: schema.ReadingModule = {
      id: testModuleId,
      title: 'To Delete',
      structuredContent: [{ index: 1, text: 'Content' }],
      paragraphCount: 1,
    ...
    it('should set isActive to false for the specified module when owner deletes', async () => {
      const expectedDeletedModule: schema.ReadingModule = { ...existingModule, isActive: false, updatedAt: expect.any(Date) };
      ...
      vi.spyOn(mockDb, 'update').mockImplementationOnce(() => ({ set: mockSetDelete }) as any);
    ...
    it('should throw forbidden error if non-admin tries to delete another admin\'s custom module', async () => {
        ...
        const moduleOwnedByOtherAdmin: schema.ReadingModule = {
          ...existingModule,
        ...
        vi.spyOn(mockDb.query.profiles, 'findFirst').mockResolvedValueOnce({ role: UserRole.ADMIN } as Partial<schema.Profile> as any);
  
        await expect(readingModuleService.deleteModule(
          'other-admin-module-delete',
          testAdminId // Current user is testAdminId
    ...
    it('should prevent a non-Super Admin from deleting a curated module', async () => {
        const curatedModule: schema.ReadingModule = {
            ...existingModule,
    ...
    it('should allow Super Admin to delete a curated module', async () => {
        const curatedModule: schema.ReadingModule = { ...existingModule, type: ModuleType.CURATED, adminId: null, genre: 'History', language: Language.US };
        const returnedDeletedModule = { ...curatedModule, isActive: false, updatedAt: new Date() };

        // Cast mock data
        vi.spyOn(mockDb.query.readingModules, 'findFirst').mockResolvedValueOnce(curatedModule as schema.ReadingModule);
        // Update profile mock
        vi.spyOn(mockDb.query.profiles, 'findFirst').mockResolvedValueOnce({ id: 'super-admin-uuid', role: UserRole.SUPER_ADMIN } as Partial<schema.Profile> as any);
        ...
        vi.spyOn(mockDb, 'update').mockImplementationOnce(() => ({ set: mockSetSADelete }) as any);

        await expect(readingModuleService.deleteModule(
            curatedModule.id,
            'super-admin-uuid' // ID representing the Super Admin
    ...
    it('should allow Super Admin to delete another admin\'s custom module', async () => {
        ...
        const moduleOwnedByOther: schema.ReadingModule = { ...existingModule, adminId: otherAdminId, type: ModuleType.CUSTOM, genre: 'History', language: Language.UK };
        const returnedDeletedModule = { ...moduleOwnedByOther, isActive: false, updatedAt: new Date() };

        // Cast mock data
        vi.spyOn(mockDb.query.readingModules, 'findFirst').mockResolvedValueOnce(moduleOwnedByOther as schema.ReadingModule);
        // Update profile mock
        vi.spyOn(mockDb.query.profiles, 'findFirst').mockResolvedValueOnce({ id: 'super-admin-uuid', role: UserRole.SUPER_ADMIN } as Partial<schema.Profile> as any);
        ...
        vi.spyOn(mockDb, 'update').mockImplementationOnce(() => ({ set: mockSetOtherSADelete }) as any);

        await expect(readingModuleService.deleteModule(
            moduleOwnedByOther.id,
            'super-admin-uuid' // ID representing the Super Admin
        )).resolves.toEqual(expect.objectContaining({ isActive: false, updatedAt: expect.any(Date) }));
    ...
    it('should allow SuperAdmin to delete any module', async () => {
      ...
      const existingModule = { ...mockReadingModule, id: curatedModuleId, type: ModuleType.CURATED, genre: 'Science' as const, language: Language.US };
      ...
      vi.spyOn(mockDb, 'update').mockImplementation(() => ({ set: mockSetSADeleteAny }) as any);

      vi.spyOn(mockDb.query.readingModules, 'findFirst').mockResolvedValue(existingModule);
      vi.spyOn(mockDb.query.profiles, 'findFirst').mockResolvedValue({ role: UserRole.SUPER_ADMIN } as Partial<schema.Profile> as any);
    ...
    it('should throw 403 if user tries to delete module they don\'t own (and not SuperAdmin)', async () => {
      ...
      vi.spyOn(mockDb.query.readingModules, 'findFirst').mockResolvedValue(existingModule as schema.ReadingModule);
      // Update profile mock
      vi.spyOn(mockDb.query.profiles, 'findFirst').mockResolvedValue({ role: UserRole.ADMIN } as Partial<schema.Profile> as any);
  ...
  describe('Admin Query Methods', () => {
    ...
    it('getAllModulesForAdmin should return all modules ordered by updatedAt desc', async () => {
        vi.spyOn(mockDb.query.readingModules, 'findMany').mockResolvedValueOnce(allModules as schema.ReadingModule[]);
    ...
    it('getCuratedModulesForAdmin should return only curated modules ordered by updatedAt desc', async () => {
        ...
        vi.spyOn(mockDb.query.readingModules, 'findMany').mockResolvedValueOnce(curatedModules as schema.ReadingModule[]);
    ...
    it('getCustomModulesForAdmin should return only custom modules with admin name, ordered by updatedAt desc', async () => {
        ...
        vi.spyOn(mockDb, 'select').mockImplementationOnce(() => ({ from: mockFromAdmin }) as any);
  ...
  describe('getModuleParagraph', () => {
    ...
    const sampleStructuredContent: Paragraph[] = [
    ...
    it('should throw an error if structuredContent is not an array', async () => {
      ...
      vi.spyOn(mockDb.query.readingModules, 'findFirst').mockResolvedValue({
        ...
        authorFirstName: null,
        authorLastName: null,
      } as unknown as schema.ReadingModule);
    ...
    it('should handle database errors gracefully', async () => {
      // Mock database error
      vi.spyOn(mockDb.query.readingModules, 'findFirst').mockRejectedValue(
  ...
  describe('Vocabulary Management', () => {
    // Explicitly import the type here if top-level isn't working
    type NewVocabularyInput = import('../services/reading.service').NewVocabularyInput;
    ...
    const mockModuleForVocab: schema.ReadingModule = {
      ...mockReadingModule,
      id: vocabModuleId,
      adminId: vocabUserId,
      type: ModuleType.CUSTOM,
    ...
    const mockVocabularyInput: NewVocabularyInput = {
    ...
    const mockVocabularyEntry: schema.Vocabulary = {
      id: vocabEntryId,
    ...
    beforeEach(() => {
      ...
      vi.mocked(mockDb.query.profiles.findFirst).mockResolvedValue({ id: vocabUserId, role: UserRole.ADMIN } as any);
    });
    ...
    describe('createVocabularyEntry', () => {
      ...
      it('should throw 403 if user is not owner/super-admin of custom module', async () => {
        ...
        vi.mocked(mockDb.query.profiles.findFirst).mockResolvedValueOnce({ id: vocabUserId, role: UserRole.ADMIN } as any);
        await expect(readingModuleService.createVocabularyEntry(vocabModuleId, vocabUserId, mockVocabularyInput))
      ...
      it('should throw 409 on unique constraint violation', async () => {
        ...
        (uniqueConstraintError as any).code = '23505'; // Attach the code property
    ...
    describe('updateVocabularyEntry', () => {
       const updateData: Partial<NewVocabularyInput> = { description: 'A new, updated description.' };
      ...
      it('should update a vocabulary entry successfully', async () => {
        ...
        vi.spyOn(mockDb, 'update').mockImplementationOnce(() => ({ set: mockSet }) as any);
    ...
    describe('deleteVocabularyEntry', () => {
      ...
      it('should delete a vocabulary entry successfully', async () => {
        ...
        vi.spyOn(mockDb, 'delete').mockImplementationOnce(() => ({ where: mockWhere }) as any);
    ...
    describe('getVocabularyForParagraph', () => {
      ...
      it('should return vocabulary entries for a specific paragraph', async () => {
        ...
        expect(mockDb.query.vocabulary.findMany).toHaveBeenCalledWith({
           where: and(
               eq(schema.vocabulary.moduleId, vocabModuleId),
               eq(schema.vocabulary.paragraphIndex, targetParagraphIndex)
           ),
