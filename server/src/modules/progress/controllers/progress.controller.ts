...
import { ProgressService, SubmitParagraphSummaryInput, CreateProgressInput, UpdateProgressInput, DetailedProgressAdminResponse } from '../services/progress.service';
import { db } from '@/db';
import { z } from 'zod';
 ...
 */
export const startProgress = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const input: CreateProgressInput = { studentId, moduleId };
    ...
    logger.error('Error in startProgress controller:', { error: (error as Error)?.message });
 ...
 */
export const submitParagraphSummary = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const studentId = req.user?.id;
    if (!studentId) {
      // Use standard error response
    ...
    const input: SubmitParagraphSummaryInput = { ...validatedBody, studentId };
    ...
    // Use standard error response for other errors
    logger.error('Error in submitParagraphSummary controller:', { error: (error as Error)?.message });
 ...
 */
export const getStudentProgressDetails = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const studentId = req.user?.id;
    if (!studentId) {
      // Use standard error response
    ...
    // Use standard error response for other errors
     logger.error('Error in getStudentProgressDetails controller:', { error: (error as Error)?.message });
 ...
 */
export const updateProgressByAdmin = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Use standard error response for other errors
    // Updated log context
    logger.error('Error in updateProgressByAdmin controller:', { error: (error as Error)?.message });
 ...
 */
export const getAllStudentProgress = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const studentId = req.user?.id;
    if (!studentId) {
      // Use standard error response
    ...
    // Use standard error response for other errors
    logger.error('Error in getAllStudentProgress controller:', { error: (error as Error)?.message });
 ...
 */
export const getAllModuleProgress = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { moduleId } = moduleIdParamSchema.parse(req.params);
    ...
    logger.error('Error in getAllModuleProgress controller:', { error: (error as Error)?.message });
 ...
 */
export const getStudentProgressForAdmin = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const adminId = req.user?.id;
    if (!adminId) {
      return res.status(401).json({ status: 'error', message: 'Admin authentication required.' });
    ...
    logger.error('Error in getStudentProgressForAdmin controller:', { error: (error as Error)?.message });
 ...
 */
export const getDetailedStudentModuleProgressForAdmin = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const adminId = req.user?.id;
    if (!adminId) {
      return res.status(401).json({ status: 'error', message: 'Admin authentication required.' });
    ...
    const detailedProgress: DetailedProgressAdminResponse = await progressService.getDetailedStudentModuleProgressForAdmin(adminId, studentId, moduleId);
    ...
    logger.error('Error in getDetailedStudentModuleProgressForAdmin controller:', { error: (error as Error)?.message });
