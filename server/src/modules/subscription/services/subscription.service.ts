...
export class SubscriptionService {
  ...
  async syncPlansFromStripe() {
    try {
      ...
      const allowedTiers: ReadonlyArray<typeof SubscriptionPlan.enumValues[number]> = SubscriptionPlan.enumValues;
      ...
      for (const plan of stripePlans) {
        ...
        if (metadataTier) {
          if ((allowedTiers as ReadonlyArray<string>).includes(metadataTier)) {
            validatedTier = metadataTier as typeof SubscriptionPlan.enumValues[number];
          } else {
    ...
    try {
      ...
      if (existingSubscription.length > 0) {
        ...
        const customer = await stripeService.createCustomer(
          user[0].email || `user+${userId}@noteearly.com`, // Ensure valid email format
      ...
      if (!stripeCustomerId) {
          if (user[0].stripeCustomerId) {
              ...
              const customer = await stripeService.createCustomer(
                user[0].email || `user+${userId}@noteearly.com`,
                user[0].fullName || undefined,
                { userId }
              );
              stripeCustomerId = customer.id;
  ...
  async cancelSubscription(userId: string) {
    try {
      ...
      const cancelledStripeSubscription = await stripeService.cancelSubscription(
        subscription[0].id // Use the actual subscription ID from the DB
      );
