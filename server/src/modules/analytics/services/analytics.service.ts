...
export class AnalyticsService {
  ...
  constructor(dbInstance: DbInstanceType = db) {
    this.db = dbInstance;
  }
   ...
   */
  async getTeacherStudentStats(teacherId: string) {
    try {
      ...
      const completedModulesQuery = await this.db
        ...
        .innerJoin(
          schema.profiles,
          eq(schema.studentProgress.studentId, schema.profiles.id)
        )
        .where(eq(schema.profiles.adminId, teacherId))
        .execute();
   ...
   */
  async getStudentProgressOverTime(studentId: string, days: number = 30) {
    try {
      ...
      const progressResults = await this.db
        ...
        .where(
          and(
            ...
            gte(schema.studentProgress.updatedAt, startDate)
          )
        )
   ...
   */
  async getPopularModules(limit: number = 10) {
    try {
      const popularModules = await this.db
        ...
        .from(schema.studentProgress)
        .innerJoin(
          schema.readingModules,
          eq(schema.studentProgress.moduleId, schema.readingModules.id)
        )
