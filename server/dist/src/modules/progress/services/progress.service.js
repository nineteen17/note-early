...
export class ProgressService {
    ...
    async startProgress(input) {
        ...
        try {
            ...
            if (!module)
                throw new AppError('Reading module not found', 404);
     ...
     */
    async submitParagraphSummary(input) {
        const { studentId, moduleId, paragraphIndex, paragraphSummary, cumulativeSummary } = input;
        // Basic validation
     ...
     */
    async updateProgress(progressId, updates, // Input type excludes summary fields
    isTeacherUpdate = false // Flag for tracking teacher feedback time
    ) {
        try {
            ...
            if (updates.completed !== undefined)
                updatesToSend.completed = updates.completed;
            if (updates.score !== undefined)
                updatesToSend.score = updates.score;
            if (updates.timeSpentMinutes !== undefined)
                updatesToSend.timeSpentMinutes = updates.timeSpentMinutes;
            if (updates.teacherFeedback !== undefined)
                updatesToSend.teacherFeedback = updates.teacherFeedback;
     ...
     */
    async getStudentProgressDetails(studentId, moduleId) {
    ...
    async getStudentModuleProgress(studentId, moduleId) {
    ...
    async getAllModuleProgress(moduleId) {
        // This method is fine
        try {
            const module = await this.db.query.readingModules.findFirst({ columns: { id: true }, where: eq(schema.readingModules.id, moduleId) });
            if (!module)
    ...
    async getTeacherStudentsProgress(teacherId) {
        try {
